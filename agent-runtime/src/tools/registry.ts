...
export const toolRegistry: RegisteredToolDefinition[] = [
  ...
  },
  {
    ...
    description: 'Delete a file inside the Nexora workspace root after explicit approval. Supports expectedSha256 optimistic locking.',
    inputSchema: objectSchema({ path: relativePathSchema, expectedSha256: stringSchema('Optional sha256 of existing content.'), confirmedByUser: approvalBooleanSchema, approvalNote: approvalNoteSchema }, ['path']),
    parameters: z.object({ path: z.string().min(1), expectedSha256: z.string().default(''), confirmedByUser: z.boolean().default(false), approvalNote: z.string().default('') }),
    scopes: ['runtime.code.write'],
    riskLevel: 'write',
    humanApprovalRequired: true,
    audit: { category: 'code', action: 'delete_file', resourceType: 'workspace_file', resourceIdField: 'path', sensitiveFields: ['path'], logEvents: ['tool.code.delete_file.approval_requested', 'tool.code.delete_file.completed'] },
